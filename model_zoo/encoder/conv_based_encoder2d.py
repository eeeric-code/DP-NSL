...
class RDN(nn.Module):
    def __init__(self, feature_dim=128, num_features=64, growth_rate=64, num_blocks=8, num_layers=3):
        ...
        self.sfe1 = nn.Conv2d(1, num_features, kernel_size=3, padding=3 // 2)
...
class ResCNN(nn.Module):
    def __init__(self, feature_dim=128,in_slice=4):
        super(ResCNN, self).__init__()
        self.conv_start = nn.Sequential(
            nn.Conv2d(in_channels=in_slice, out_channels=64, kernel_size=3, padding=3 // 2),
        ...
        self.conv_end = nn.Sequential(
            nn.Conv2d(in_channels=64, out_channels=feature_dim * in_slice, kernel_size=3, padding=3 // 2),
        )
    ...
    def forward(self, x):
        ...
        # x + res_img
        res_img = res_img.reshape(b,-1,d,h,w).permute(0,1,3,4,2)
        return res_img
...
def build_rescnn2d(args):
    """Build ResCNN model based on the provided arguments."""
    # args = Namespace(**args)  # Convert dict to Namespace for easier attribute access
    return ResCNN(feature_dim=getattr(args, 'feature_dim', 128))

def build_rdn2d(args):
    """Build RDN model based on the provided arguments."""
    # args = Namespace(**args)  # Convert dict to Namespace for easier attribute access
    return RDN(feature_dim=getattr(args, 'feature_dim', 128),
               num_features=getattr(args, 'num_features', 64),
               growth_rate=getattr(args, 'growth_rate', 64),
               num_blocks=getattr(args, 'num_blocks', 8),
               num_layers=getattr(args, 'num_layers', 3))

def build_srresnet2d(args):
    """Build SRResNet model based on the provided arguments."""
    # args = Namespace(**args)  # Convert dict to Namespace for easier attribute access
    return SRResnet(nf=getattr(args, 'nf', 64),
